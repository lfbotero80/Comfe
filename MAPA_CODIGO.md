...
### 3.41 — El riesgo por sede se ve, no se lee, en `SPRINT-43` y `SPRINT-44`

Dos sprints sobre el mismo bloque, por dos problemas distintos.

`SPRINT-43` atendio un reclamo de lectura de Luis Felipe sobre el plano cartesiano ("no la entiendo, no se lee facil") y al revisarlo aparecieron tres defectos reales: etiquetas de sede que colisionaban (cuatro sedes compartiendo dos nombres cortos), linea de meta presupuestal dibujada en 108% en vez de 90%, y sedes **sin dato** ubicadas justo en la esquina rotulada "Accion prioritaria" — indistinguibles de una sede en crisis. Se elimino el cuadrante y su CSS (20 reglas, ninguna reutilizada), y `dashboard-command.js` gano `riskGroupFor()` y `BUDGET_TARGET`, que clasifican cada sede en `both` / `occupancy` / `budget` / `ok` / `insufficient`.

`SPRINT-44` corrigio el exceso del anterior: el reemplazo era correcto pero estaba escrito en texto, y Luis Felipe lo rechazo ("eso no se parece a un dashboard visual, eso es demasiado texto"). El bloque paso a ser un grafico de barras divergentes: la clasificacion del dominio no cambio, cambio como se muestra. El tipo de riesgo se reconoce por la **forma** de la fila, no por una etiqueta escrita, y las sedes sin dato usan barra rayada con `s/d`.
